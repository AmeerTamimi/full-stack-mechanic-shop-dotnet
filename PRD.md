...
The Mechanic Shop Management System , is An operational platform designed to Manage and Handle : 
...
It will support day-to-day repair shop tasks , by centrlizing the source of turth for customers , vehicles , work order tracking , techinican scheduling , and service execution. It will handle scheduling conflicts , provide a clear and visible task plans. Providing a consistent high quality daily services.

## Problem Statement - Why this system Exists ?

Many repair shops still rely on paper notes or human memory to manage daily operations. This creates Issue , such as : 
...
4. **Fast Customer & Vehicle Data Retrieval :** Make it easy to find any customer, vehicle, or past job history instantly.
5. **Simplify Scheduling :** Provide an easy scheduling workflow (daily view + technician view) that makes planning quick and clear.
6. **Increase Service Quality & Efficiency :** Standardize repair tasks and improve workflow clarity using pre-made templates ,  to reduce mistakes and speed up completion.
7. **Enable Better Decisions Through Tracking & Reporting :** Provide basic metrics (Scheduled / InProgress / Completed / Cancelled) to support better planning and management.
...
1. You cannot remove a customer who has a work order (scheduled / in progress).
...
2. A technician can only see work orders assigned to them (not other technicians’ work orders).
...
1. schedule based on date , time , service bay
2. view daily scehdules
3. Assign A Technician to a work order
4. update it (time , techni , service bay)
5. remove it

### The Technician Can Do :
...
1. no conflict
2. no double booking
3. cant remove technician from a schedule while hes in progress 
4. You can update the schedule while it’s **InProgress**, but **only limited fields**:
    ...
    - Not allowed: change **time** or **technician** (because the job already started)
...
5. Sessions must expire after period of inactivity
...
3. Users see only features they have permission to access
...
- **Stakeholders:** Its actually only me now , hope some real customers in the future ..